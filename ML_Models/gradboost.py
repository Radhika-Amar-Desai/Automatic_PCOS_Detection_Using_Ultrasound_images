...
#dataset_dir = 'non_processed_images'
...
# Create and train the Gradient Boosting classifier with PCA
grad_boost_classifier = GradientBoostingClassifier(n_estimators=100, random_state=42)
grad_boost_classifier.fit(X_train_pca, y_train)

# Make predictions on the test set using the reduced data
y_pred = grad_boost_classifier.predict(X_test_pca)
